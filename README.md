...
To get started with StudyNotion locally, follow these steps:

1. **Clone the repository.**
2. **Setup Server:**
   - Navigate to the `Server` directory.
   - Run `npm install` to install dependencies.
   - Create a `.env` file based on `.env.example` and fill in your credentials (MongoDB, Cloudinary, Razorpay, Mail).
   - **Seed the Database:** Run `npm run seed` to populate course categories.
   - Run `npm run dev` to start the backend server.
3. **Setup Client:**
   - Navigate to the `Client` directory.
   - Run `npm install` to install dependencies.
   - Create a `.env` file based on `.env.example` and set `REACT_APP_BASE_URL`.
...
- Shivam Rajbhar
  - GITHUB: [Shivam Rajbhar](https://github.com/Itshivamgithub)
...
For any queries or suggestions, feel free to contact the author:
- Shivam Rajbhar: [Email](mailto:shivamrajbhar883@gmail.com)
